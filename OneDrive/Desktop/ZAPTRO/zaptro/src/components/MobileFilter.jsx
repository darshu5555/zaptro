...
const MobileFilter = ({openFilter, setOpenFilter,search, setSearch, brand, setBrand,priceRange, setPriceRange, category, setCategory, handleBrandChange, handleCategoryChange  }) => {
  ...
  return (
    <>
    <div className='bg-gary-100 flex justify-between items-center md:hidden px-4 p-2 mt-5'>
...
openFilter ? <div className='bg-gray-100 p-2 md:hidden'>
  <input type='text' placeholder='Search..' 
        value={search} onChange={(e)=>setSearch(e.target.value)}className='bg-white p-2 rounded-md border-gray-400 border-2 w-full'/>
   {/* category only data */}
...
categoryOnlyData?.map((item, index)=>{
  return <div key={index} className='flex gap-2'>
    ...
    <button className='cursor-pointer uppercase'>{item}</button>
      ...
      <select name='' id='' className='bg-white w-full p-2 border-gray-200 border-2 rounded-md uppercase'
       value={brand}
...
<button className='bg-red-500 text-white rounded-md px-3 py-1 mt-5 cursor-pointer' onClick={()=>{setSearch(''); setCategory('All'); setBrand('All'); setPriceRange([0,5000]); setOpenFilter(false)}}>Reset Filters</button>
</div>: null
    }
    </>
  )
